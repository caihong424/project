...
import org.eastwill.model.GetChkOutRpListReqModel;
...
public class GetCheckOutReportListService {
	private static final Log log=LogFactory.getLog(GetCheckOutReportListService.class);
	
	public static String getCheckOutReportList(String strXml, String res_xml) throws Exception {
		...
		String toutstr = "" ;
		//检查/检验列表查询接口
		GetChkOutRpListReqModel objreq = XmlUtils.toBean(AesUtils.decrypt(Config.getProper("key"),obj.getReqEncrypted()),GetChkOutRpListReqModel.class);
		String turl = Config.getProper("url")+"/GetCheckOutReportList";
		String thosId = objreq.getHosId();
		String thospPatientId = objreq.getHospPatientId();
		String tpatientIdcardType = objreq.getPatientCardType();
		String tpatientIdcrdNo = objreq.getPatientIdcardNo();
		...
		String tinstr = thosId+"|"+thospPatientId+"|"+tpatientIdcardType+"|"+tpatientIdcrdNo+"|"+tpatientCardType+"|"+tpatientCardNo+"|"+tpatientName+"|"+tpatientSex+"|"+tpatientAge+"|"+ tbeginDate+"|"+tendDate;
		log.info("传入检查/检验列表查询接口"+tinstr);
		...
		try {
			...
			log.info("检查/检验列表查询接口信息2:"+toutdata);
			String terrMsg = thout.getErrorMsg();
			if(tresultCode.equals("0")) {
				log.info("获取his检查/检验列表查询接口信息成功"+toutdata);
			}else {
				log.info("获取his检查/检验列表查询接口信息失败"+terrMsg);
				return MessageFormat.format(res_xml, "-3", "获取his检查/检验列表查询接口信息失败"+terrMsg, "MD5", res_sign, res_encrypted);
			}
		
			toutstr = toutdata;
		}catch(Exception e) {
			log.error("3:获取his检查/检验列表查询接口信息失败"+e);
			return MessageFormat.format(res_xml, "-3", "获取his检查/检验列表查询接口信息失败"+e.getMessage(), "MD5", res_sign, res_encrypted);
		}
		log.info("返回检查/检验列表查询接口信息:"+toutstr);
