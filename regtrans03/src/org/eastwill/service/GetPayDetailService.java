...
import org.eastwill.model.PayDetailReqModel;
...
public class GetPayDetailService {
	private static final Log log=LogFactory.getLog(GetPayDetailService.class);
	
	public static String getPayDetail(String strXml, String res_xml) throws Exception {
		...
		String toutstr = "" ;
		//缴费明细查询接口
		PayDetailReqModel objreq = XmlUtils.toBean(AesUtils.decrypt(Config.getProper("key"),obj.getReqEncrypted()),PayDetailReqModel.class);
		String turl = Config.getProper("url")+"/GetPayDetail";
		String thosId = objreq.getHosId();
		String thospPatientId = objreq.getHospPatientId();
		String thospSequence = objreq.getHospSequence();
		String tinstr = thosId+"|"+thospPatientId+"|"+thospSequence;
		log.info("传入缴费明细查询接口信息"+tinstr);		
		try {
			...
			log.info("缴费明细查询接口信息2:"+toutdata);
			String terrMsg = thout.getErrorMsg();
			if(tresultCode.equals("0")) {
				log.info("获取his缴费明细查询接口信息成功"+toutdata);
			}else {
				log.info("获取his缴费明细查询接口信息失败"+terrMsg);
				return MessageFormat.format(res_xml, "-3", "获取his缴费明细查询接口信息失败"+terrMsg, "MD5", res_sign, res_encrypted);
			}
						
			toutstr = toutdata;
		}catch(Exception e) {
			log.error("3:获取his缴费明细查询接口信息失败"+e);
			return MessageFormat.format(res_xml, "-3", "获取his缴费明细查询接口信息失败"+e.getMessage(), "MD5", res_sign, res_encrypted);
		}
		log.info("返回缴费明细查询接口信息:"+toutstr);
