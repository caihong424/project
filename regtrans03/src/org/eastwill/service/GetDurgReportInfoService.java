...
import org.eastwill.model.GetDurgRpInfoReqModel;
...
public class GetDurgReportInfoService {
	private static final Log log=LogFactory.getLog(GetDurgReportInfoService.class);
	
	public static String getDurgReportInfo(String strXml, String res_xml) throws Exception {
		...
		String toutstr = "" ;
		//检验报告查询(药敏检验)接口
		GetDurgRpInfoReqModel objreq = XmlUtils.toBean(AesUtils.decrypt(Config.getProper("key"),obj.getReqEncrypted()),GetDurgRpInfoReqModel.class);
		String turl = Config.getProper("url")+"/GetDurgReportInfo";
		...
		log.info("传入检验报告查询(药敏检验)接口信息"+tinstr);
		...
		try {
			...
			log.info("检验报告查询(药敏检验)接口信息2:"+toutdata);
			String terrMsg = thout.getErrorMsg();
			if(tresultCode.equals("0")) {
				log.info("获取his检验报告查询(药敏检验)接口信息成功"+toutdata);
			}else {
				log.info("获取his检验报告查询(药敏检验)接口信息失败"+terrMsg);
				return MessageFormat.format(res_xml, "-3", "获取hisv信息失败"+terrMsg, "MD5", res_sign, res_encrypted);
			}
			
			toutstr = toutdata;
		}catch(Exception e) {
			log.error("3:获取his检验报告查询(药敏检验)接口信息失败"+e);
			return MessageFormat.format(res_xml, "-3", "获取his检验报告查询(药敏检验)接口信息失败"+e.getMessage(), "MD5", res_sign, res_encrypted);
		}
		log.info("返回检验报告查询(药敏检验)接口信息:"+toutstr);
